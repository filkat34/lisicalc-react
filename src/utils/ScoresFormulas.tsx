...
//Décompte du nombre de caractères
  export function nbCaracteres(text: string): number {
    let regex = /(\w)/gi;
  ...
  export function nbVoyelles(text: string): number {
    let regex = /[aeiouœ]/gi;
 ...
 export function nbDigrammes(text: string): number {
  ...
  export function nbTrigrammes(text: string): number {
  ...
  export function nbSyllabesGraphiques(text: string): number {
    return nbVoyelles(text) - (nbDigrammes(text) + nbTrigrammes(text));
  }

  //Décompte du nombre de mots
 export function nbMots(text: string): number {
    let regex = /(\w+)/gi;
  ...
  export function nbMotsPolysyllabiques(text: string): number {
    let punctuation = /[.,?!;:]/g;
  ...
  export function nbMotsLongs(text: string): number {
    let regex = /(\w{7,})/g;
  ...
  //Décompte des phrases
   export function nbPhrases(text: string): number {
    let regex = /..\.|!|\?/gi;
  ...
  //Calcul Automated Readability Index
   export function scoreAri(text: string): number {
   ...
   * @returns 
   */
   export function scoreAnalysis(formula: string, score: number): string {
